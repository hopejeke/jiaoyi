...
public class CouponController {
    ...
    public ResponseEntity<ApiResponse<Boolean>> validateCoupon(
        ...
        boolean isValid = couponService.validateCoupon(couponOpt.get(), userId, orderAmount, null);
     ...
     * 计算优惠金额
     */
    @PostMapping("/{couponCode}/calculate")
    ...
    public ResponseEntity<ApiResponse<Void>> useCoupon(
        ...
        log.info("使用优惠券，代码: {}, 用户ID: {}, 订单ID: {}, 优惠金额: {}", couponCode, userId, orderId, discountAmount);
        
        try {
            couponService.useCoupon(couponCode, userId, orderId, discountAmount);
            return ResponseEntity.ok(ApiResponse.success("使用成功", null));
        } catch (Exception e) {
            log.error("使用优惠券失败", e);
            return ResponseEntity.ok(ApiResponse.error(400, "使用失败: " + e.getMessage()));
        }
    }
