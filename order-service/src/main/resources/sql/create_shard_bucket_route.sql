...
CREATE TABLE IF NOT EXISTS shard_bucket_route (
    bucket_id INT NOT NULL PRIMARY KEY COMMENT '虚拟分片ID（0-1023，对应 shard_id）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='分片路由表（bucket_id → 物理库映射）';
...
INSERT INTO shard_bucket_route (bucket_id, ds_name, status) VALUES
(0, 'ds0', 'NORMAL'),
(1, 'ds0', 'NORMAL'),
(2, 'ds0', 'NORMAL')
-- ... 其他 bucket_id 0-341 都映射到 ds0
ON DUPLICATE KEY UPDATE ds_name = VALUES(ds_name), status = VALUES(status);

-- 使用存储过程批量初始化（更高效）
DELIMITER $$

CREATE PROCEDURE IF NOT EXISTS init_shard_bucket_route()
BEGIN
    ...
    WHILE i < 1024 DO
        ...
        INSERT INTO shard_bucket_route (bucket_id, ds_name, status)
