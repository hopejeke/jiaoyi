...
public class ProductRouteCache {
    ...
    public void init() {
        ...
        try {
            ...
            if (routeMap.size() != 1024) {
                throw new IllegalStateException(
                    String.format("路由数据不完整，期望 1024 条，实际 %d 条", routeMap.size()));
            }
    ...
    private void loadAllRoutes() {
        String sql = "SELECT bucket_id, ds_name, status, updated_at FROM product_shard_bucket_route ORDER BY bucket_id";
    ...
    public void refreshRoutes() {
        ...
        try {
            ...
            if (maxUpdatedAt != null && (currentLastUpdateTime == null || maxUpdatedAt.isAfter(currentLastUpdateTime))) {
                ...
                try {
                    if (routeMap.size() != 1024) {
                        log.error("【ProductRouteCache】刷新后路由数据不完整，期望 1024 条，实际 {} 条", routeMap.size());
                        // 数据不完整，降级为全量加载
                        lock.readLock().unlock();
                        loadAllRoutes();
                        lock.readLock().lock();
    ...
    private boolean loadIncrementalRoutes(LocalDateTime lastUpdateTime) {
        ...
        try {
            String sql = "SELECT bucket_id, ds_name, status, updated_at FROM product_shard_bucket_route " +
                        "WHERE updated_at > ? ORDER BY bucket_id";
            ...
            try {
                ...
                // 验证数据完整性（必须包含 1024 条）
                if (routeMap.size() != 1024) {
                    log.warn("【ProductRouteCache】增量刷新后数据不完整，期望 1024 条，实际 {} 条，需要全量加载", routeMap.size());
                    return false;
                }
     ...
     * 
     * @param productShardId 分片ID（0-1023）
     * @return 状态（NORMAL/MOVING/DUAL_WRITE）
     */
    ...
    public boolean forceRefresh() {
        ...
        try {
            ...
            try {
                if (routeMap.size() != 1024) {
                    log.error("【ProductRouteCache】强制刷新后路由数据不完整，期望 1024 条，实际 {} 条", routeMap.size());
                    return false;
                } else {
