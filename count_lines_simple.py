...
def count_directory(extensions, exclude_dirs=['target', 'node_modules', '.git', '__pycache__', '.idea']):
    total_lines = 0
    file_count = 0
...
java_lines, java_files = count_directory(['.java'])
print(f"Java文件: {java_files:4d} 个, {java_lines:7,} 行")

xml_lines, xml_files = count_directory(['.xml'])
print(f"XML文件:  {xml_files:4d} 个, {xml_lines:7,} 行")

prop_lines, prop_files = count_directory(['.properties'])
print(f"Properties文件: {prop_files:4d} 个, {prop_lines:7,} 行")

yaml_lines, yaml_files = count_directory(['.yml', '.yaml'])
print(f"YAML文件: {yaml_files:4d} 个, {yaml_lines:7,} 行")

sql_lines, sql_files = count_directory(['.sql'])
...
if os.path.exists(oo_path):
    ...
    ts_lines, ts_files = count_directory(['.ts', '.js'])
    print(f"TS/JS文件: {ts_files:4d} 个, {ts_lines:7,} 行")
    
    json_lines, json_files = count_directory(['.json'])
